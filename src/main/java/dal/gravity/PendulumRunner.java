...
public class PendulumRunner {
	...
	GravityConstant planet1 = new GravityConstant(8.3452);
	GravityConstant planet2 = new GravityConstant(10.3452);
	GravityConstant planet3 = new GravityConstant(0.3452);
	//double gravity = 9.80665;
	double delta = (args.length == 0) ? .1 : Double.parseDouble (args[0]);
	double sLen = 10, pMass = 10, theta0 = Math.PI/30;
	RegularPendulum rp = new RegularPendulum (sLen, pMass, theta0, delta, planet1);
	SimplePendulum sp = new SimplePendulum (sLen, pMass, theta0, planet1);
	RegularPendulum rpCoarse = 
		new RegularPendulum (sLen, pMass, theta0, .1, planet1);
	...
	for (int second = 1; second <= 20; second++) {
	    if (second = 15) 
		rp.setGravityModel(planet2);
	    if (second = 12) 
		sp.setGravityModel(planet3);
